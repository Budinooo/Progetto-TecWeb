...
class Community_Feed extends React.Component {
    ...
    getPosts = () =>{
        fetch('http://site212229.tw.cs.unibo.it/db/collection?collection=communityFeed',{
            method:'GET'
    ...
    sendForm = () => { //da fare con mongo 
        ...
        if(this.state.isAdd && this.state.isPost){ //send answer
            ...
            if (desc){
                ...
                let date = new Date();
                fetch('http://site212229.tw.cs.unibo.it/db/element?id='+login.id+'&collection=users',{
                    method:'GET'
                })
                .then(response => response.json())
                .then(data => {
