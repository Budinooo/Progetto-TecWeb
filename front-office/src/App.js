...
class App extends React.Component {
  ...
  componentDidMount()
  {    
    fetch('http://localhost:8000http://localhost:8000/db/collection?collection=products', {method:"GET"}).then((res)=>res.json()).then((data)=> {this.setState({products: data.result.slice(0,8)})});
    fetch('http://localhost:8000http://localhost:8000/db/element?id=0&collection=services', {method: "GET"}).then((res) => res.json()).then((data) => this.setState({service: data.result}));
